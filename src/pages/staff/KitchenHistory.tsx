      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            A complete record of all your past kitchen orders.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex-1 flex flex-col">
        {historyOrders.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-slate-500 text-sm">Your past orders will appear here.</p>
          </div>
        ) : (
                ...
                <div key={order._id} className="p-5 bg-slate-50 hover:bg-slate-100 transition-colors rounded-xl border border-slate-200 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                      ...
                      order.status === "rejected" ? "bg-red-100 text-red-700" : "bg-slate-200 text-slate-600"
                    }`}>
                      {order.status}
                    </span>
                  </div>
                  ...
                  <div className="text-sm text-slate-600 space-y-1 mb-4 flex-1">
                    ...
                    {order.items.length > 4 && (
                      <div className="text-xs italic text-slate-400 mt-1">+{order.items.length - 4} more items</div>
                    )}
                  </div>
                </div>
              ))}
